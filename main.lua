...
-- 依赖 WindUI 框架，需在原有 main.lua 环境中运行
local WindUI = require(...) -- 若已加载 WindUI 可直接使用
local TweenService = game:GetService("TweenService")

-- 1. 定义彩色循环颜色序列（可自定义颜色和关键帧）
local function createRainbowGradient()
    local colorKeypoints = {
        ColorSequenceKeypoint.new(0, Color3.fromHex("#ff0000")),   -- 红
        ColorSequenceKeypoint.new(0.2, Color3.fromHex("#ffff00")), -- 黄
        ColorSequenceKeypoint.new(0.4, Color3.fromHex("#00ff00")), -- 绿
        ColorSequenceKeypoint.new(0.6, Color3.fromHex("#00ffff")), -- 青
        ColorSequenceKeypoint.new(0.8, Color3.fromHex("#0000ff")), -- 蓝
        ColorSequenceKeypoint.new(1, Color3.fromHex("#ff00ff"))    -- 紫
    }
    return ColorSequence.new(colorKeypoints)
end

-- 2. 给目标 UI 元素添加彩色循环效果
-- 参数：targetUI（目标UI元素，如Frame/ImageLabel）、duration（循环周期，默认2秒）
function addRainbowLoop(targetUI, duration)
    duration = duration or 2

    -- 创建渐变组件（若目标已有则复用）
    local gradient = targetUI:FindFirstChildOfClass("UIGradient")
    if not gradient then
        gradient = WindUI.Creator.New("UIGradient", {}, {Parent = targetUI})
    end

    -- 设置初始渐变（旋转90度实现横向/纵向环绕）
    gradient.Rotation = 90
    gradient.Color = createRainbowGradient()

    -- 定义循环动画（通过偏移颜色序列实现流动效果）
    ...
    -- 动画目标：偏移颜色序列（关键帧整体后移1单位，实现循环）
    local tweenGoal = {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromHex("#ff00ff")), -- 起始为原序列末尾颜色
            ColorSequenceKeypoint.new(0.2, Color3.fromHex("#ff0000")),
            ColorSequenceKeypoint.new(0.4, Color3.fromHex("#ffff00")),
            ColorSequenceKeypoint.new(0.6, Color3.fromHex("#00ff00")),
            ColorSequenceKeypoint.new(0.8, Color3.fromHex("#00ffff")),
    ...
    return {
        Tween = tween,
        Pause = function() tween:Pause() end,
        Resume = function() tween:Play() end,
        Stop = function() tween:Cancel() end
    }
end

-- 3. 示例：给 WindUI 窗口标题栏添加彩色循环
...
-- 给标题栏文本添加彩色环绕
local titleLabel = window.UIElements.Main.Main.Topbar.Left.Title.Title
local rainbowController = addRainbowLoop(titleLabel, 1.5) -- 1.5秒一圈

-- 可选：给窗口边框添加彩色循环
local windowBorder = window.UIElements.Main.Background:FindFirstChildOfClass("UIStroke")
if not windowBorder then
    windowBorder = WindUI.Creator.New("UIStroke", {
        Thickness = 2,
        Parent = window.UIElements.Main.Background
    })
end
addRainbowLoop(windowBorder, 2)
